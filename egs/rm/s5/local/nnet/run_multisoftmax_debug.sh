...
if [ $stage -le 0 ]; then
  ...
  utils/subset_data_dir_tr_cv.sh --cv-spk-percent 10 $train ${train}_tr90 ${train}_cv10 || exit 1   # ${train}_cv10 is the CV set
  # Merge-in the wsj set with train-set,
  utils/combine_data.sh $train_tr90_wsj ${train}_tr90 $wsj || exit 1  # Combine ${train}_tr90 and $wsj and save in $train_tr90_wsj. $train_tr90_wsj is the training set.
fi
comment
...
echo "ali1_pdf = $ali1_pdf"

#
: << 'comment'
if [ $stage -le 1 ]; then
  mkdir -p $dir/log
  copy-int-vector "ark:gunzip -c ${wsj_ali}/ali.*.gz |" ark,t:- | awk -v prefix=wsj '{ $1=prefix $1; print; }' | \
  ...
  featlen="ark:feat-to-len 'scp:cat $train/feats.scp $wsj/feats.scp |' ark,t:- |"   # prints number of frames for every utterance in feats.scp
  post1=scp:$dir/post1.scp
  post2=scp:$dir/post2.scp
  #echo -e "featlen = $featlen\n
  #		   post1   = $post1\n
  #		   post2   = $post2"	
