...
wsj_dev=data-fbank-blocksoftmax/test_wsj_dev93

dev_original=data/test
train_original=data/train
wsj_original=../../wsj/s5/data/train_si284
wsj_original_dev=../../wsj/s5/data/test_dev93 
...
wsj_graph=../../wsj/s5/exp/tri4b/graph_bd_tgpr
[ ! -e $wsj_graph ] && echo "Missing $wsj_graph" && exit 1
...
if [ $stage -le 1 ]; then
  ...
  copy-int-vector "ark:gunzip -c ${wsj_ali}/ali.*.gz |" ark,t:- | awk -v prefix=wsj_ '{ $1=prefix $1; print; }' | \
    gzip -c >$dir/ali_wsj.gz 

  # Store single-stream posteriors to disk, indexed by 'scp' for pasting w/o caching,
  ali-to-pdf ${gmm}_ali/final.mdl "ark:gunzip -c ${gmm}_ali/ali.*.gz |" ark:- | \
    ali-to-post ark:- ark,scp:$dir/post1.ark,$dir/post1.scp
  ali-to-pdf ${wsj_ali}/final.mdl "ark:gunzip -c $dir/ali_wsj.gz |" ark:- | \
...
# Decode the WSJ from the network, 2nd block of the <BlockSoftmax>,
# Requires updated nnet/decode.sh from JSALT workshop (not merged-in yet),
if [ $stage -le 3 ]; then
  # Calculate prior-counts from wsj,
  analyze-counts "ark:ali-to-pdf $wsj_ali/final.mdl 'ark:gunzip -c ${wsj_ali}/ali.*.gz |' ark:- |" $dir/wsj_class_frame_counts
  # Get test set of wsj,
  utils/copy_data_dir.sh $wsj_original_dev $wsj_dev
  # Make features,
  steps/make_fbank_pitch.sh --nj 10 --cmd "$train_cmd -tc 10" \
    $wsj_dev $wsj_dev/log $wsj_dev/data
  # Recognition network,
  steps/nnet/decode.sh --nj 20 --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt 0.1 \
    --blocksoftmax-dims "$ali1_dim,$ali2_dim" --blocksoftmax-active 2 \
    --model $wsj_ali/final.mdl --class-frame-counts $dir/wsj_class_frame_counts \
    $wsj_graph $dev $dir/decode_wsj_dev93
fi
