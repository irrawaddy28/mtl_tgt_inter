...
:<<'END'
  ...
  graphdir=$srcdir
END
else # Generate the training graphs using HCL.PX (no self-loop) where PX comes from probabilistic transcriptions
  graphdir=$dir
  if [ $stage -le 0 ]; then
...
# Use the training graphs (no self-loops) to generate the alignments
if [ $stage -le 1 ]; then
...
# Use the alignments to compute the FMLLR xforms
if [ $stage -le 2 ]; then
...
# Use FMLLR xforms to compute features. Use feats and HCL.PX to generate the posteriors
