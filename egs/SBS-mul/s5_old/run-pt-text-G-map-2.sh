...
dir_fsts=data_${TEST_LANG}_pt-2
...
for L in $TEST_LANG; do
  ...
  if [ $stage -le 3 ]; then
    ...
    graph_dir=$exp_dir/$L/graph_text_G
    echo "graph dir = $graph_dir"
    [[ -d $graph_dir ]] || { mkdir -p $graph_dir; utils/mkgraph.sh $dir_lang $exp_dir $graph_dir || exit 1; }    
    
	steps/decode_fmllr.sh --nj 4 --cmd "$decode_cmd" $graph_dir data/$L/dev \
		$exp_dir/decode_dev_text_G_${L}  &

	steps/decode_fmllr.sh --nj 4 --cmd "$decode_cmd" $graph_dir data/$L/eval \
		$exp_dir/decode_eval_text_G_${L} &    
  fi    
