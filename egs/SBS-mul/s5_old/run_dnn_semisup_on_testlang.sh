...
#   ./run_dnn_semisup.sh --stage 0 --gmmdir exp/tri3b_map_SW_pt --alidir exp/tri3b_ali  --graph-dir exp/tri3b_map_SW_pt/SW/graph_oracle_LG --dbn exp/dnn4_pretrain-indbn/6.dbn  --dnndir exp/dnn4_pretrain-indbn_dnn/nnet_SW_pt_optl2_xentit_5_l2_-8.5 --dir exp/dnn5_pretrain-dbn_dnn_semisup/nnet_SW_pt_semisup
...
alidir=exp/tri3b                    # dir to obtain one-hot alignments for labeled data
data_fmllr=data-fmllr-tri3b         # Directory to store fMLLR feats of unlabeled data
graph_dir=$gmmdir/$LANG/graph_oracle_LG    # Graph directory used while decoding unlabeled data and dev data
...
if [ $stage -le 0 ]; then
  steps/nnet/decode.sh $parallel_opts --nj $decode_nj --cmd "$decode_cmd" --config conf/decode_dnn.config \
...
if [ $stage -le 5 ]; then
  ...
  utils/combine_data.sh $dir/data_semisup_4k_${num_copies}x $data_fmllr/unsup_4k_$L  || exit 1
  #utils/copy_data_dir.sh $data_fmllr/${LANG}_pt/train_cv10  $data_fmllr/train_cv10_0 || exit 1
  
  sort -k1,1 $postdir/unsup_post.scp  > $dir/all_post.scp
  sort -k1,1 $postdir/unsup_frame_weights.scp > $dir/all_frame_weights.scp

  num_tgt=$(hmm-info --print-args=false $gmmdir/final.mdl | grep pdfs | awk '{ print $NF }')
  $cuda_cmd $dir/log/train.log \
    steps/nnet/train.sh --dbn $dbn \
    --cmvn-opts "--norm-means=true --norm-vars=true" \
    --delta-opts "--delta-order=2" --splice 5 \
    --hid-layers 0 --learn-rate 0.008 --num-tgt $num_tgt \
    --labels-trainf scp:$dir/all_post.scp --frame-weights scp:$dir/all_frame_weights.scp \
    $dir/data_semisup_4k_${num_copies}x $data_fmllr/${LANG}_pt/train_cv10 \
...
if [ $stage -le 7 ]; then
  ...
  for lang in $L; do
    steps/nnet/decode.sh $parallel_opts --nj $decode_nj --cmd "$decode_cmd" --config conf/decode_dnn.config --acwt $acwt \
