...
while [ $x -lt $num_iters ]; do
  ...
  if [ $stage -le $x ]; then
    echo "Pass $x: Acc stats and do MAP update"
    $cmd JOB=1:$nj $dir/log/acc.$x.JOB.log \
      gmm-acc-stats --binary=$binary $dir/$x.mdl "$feats" \
      "ark,s,cs:gunzip -c $dir/post.JOB.gz|" $dir/$x.JOB.acc || exit 1;
    [ `ls $dir/$x.*.acc | wc -w` -ne "$nj" ] && echo "$0: Wrong #accs" && exit 1;
    if $realigned_this_iter; then
		map_opts="mvw"
	else
		map_opts=""
    fi
    echo "MAP options: $map_opts"    
    $cmd $dir/log/update.$x.log \
      gmm-est-map --binary=$binary ${map_opts:+ --update-flags=$map_opts} --write-occs=$dir/$[$x+1].occs $dir/$x.mdl \
...
if [ $stage -le $x ]; then
  ...
  $cmd $dir/log/est_alimdl.log \
    gmm-est-map --update-flags=mvw $dir/$x.mdl \
