...
bn_layer=	   #which layer is the bottleneck layer		
bn_dim=		   #dimension of the bottleneck layer
...
[ ! -z "$bn_layer" ] && [ -z "$bn_dim" ] && echo "Bottleneck layer specified, but dimension is missing" && exit 1;
...
num_hid=$hid_dim
echo "dim of input feats = $num_fea, dim of hidden layers = $num_hid"
...
for depth in $(seq 1 $nn_depth); do
  ...
  if [ "$depth" == "1" ]; then
    ...
    #initialize
    echo "Initializing '$RBM.init'"
    if [[ "$depth" -eq "$bn_layer" ]]; then
		echo "<NnetProto>
		<Rbm> <InputDim> $num_hid <OutputDim> $bn_dim <VisibleType> bern <HiddenType> bern <ParamStddev> $param_stddev <VisibleBiasCmvnFilename> $dir/$depth.cmvn
		</NnetProto>
		" > $RBM.proto		
		num_hid=$bn_dim
		out_dim=$hid_dim
		...
		echo "<NnetProto>
		<Rbm> <InputDim> $num_hid <OutputDim> $out_dim <VisibleType> bern <HiddenType> bern <ParamStddev> $param_stddev <VisibleBiasCmvnFilename> $dir/$depth.cmvn
		</NnetProto>
		" > $RBM.proto
		num_hid=$hid_dim
		out_dim=$hid_dim
  ...
  if [ "$depth" == "1" ]; then
	...
	if [[ "$depth" -eq "$bn_layer" ]]; then
	  # rbm-convert-to-nnet, by default, puts the learned RBM weights and biases
	  # under the <AffineTransform> component and appends a <Sigmoid> component.
	  # To use a bottleneck layer which is a linear transform with no sigmoid, follow these steps: 
	  # Step 1: Replace "<AffineTransform>" by "<LinearTransform>" 
	  # Step 2: In the next line, add "<LearnRateCoef> 1"
	  # Step 3: Remove the bias component in the second last line
	  # Step 4: Remove the sigmoid component in the last line
	  rbm-convert-to-nnet --binary=false $RBM - | \
	  sed -e 's/AffineTransform/LinearTransform/g' -e '/LinearTransform/a <LearnRateCoef> 1' | head -n -2 - | \
