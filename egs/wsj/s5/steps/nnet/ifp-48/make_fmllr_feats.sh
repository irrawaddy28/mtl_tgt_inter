...
if [ $# != 5 ]; then
   ...
   echo "  --keytag-opts   <string>							# tags  to prepend or append keys (uttid)"
   exit 1;
fi
...
feat_dim=$(feat-to-dim --print-args=false "$(echo $feats | sed 's|JOB|1|g')" -)
echo "$0: feats = $feats, dim = $feat_dim";

# Prepare the output dir,
utils/copy_data_dir.sh ${keytag_opts} $srcdata $data; rm $data/{feats,cmvn}.scp 2>/dev/null
