...
#cp $dir/final.mdl $dir/mce
#cp $dir/final.feature_transform $dir/mce

steps/nnet/train_scheduler_mce.sh \
  ${feature_transform:+ --feature-transform $feature_transform} \
  --learn-rate $learn_rate \
  --randomizer-seed $seed \
  --max-iters ${train_mce_iters}  \
  --minibatch-size ${minibatch_size} \
  --eta-mce ${eta_mce} \
  ${train_opts} \
  ${train_tool:+ --train-tool "$train_tool"} \
  ...
  $nnet_init "$feats_tr" "$feats_cv" "$labels_tr" "$labels_cv" $dir/mce || exit 1
...
(cd $dir; rm final.nnet; ln -s mce/final.nnet final.nnet) 
...
#mv $dir/prior_counts $dir/ali_train_pdf.counts
#echo "Priors saved in $dir/ali_train_pdf.counts"
