...
if [ $# != 3 ]; then
   ...
   echo "  --verbose <n>			    # verbose option for gmm-latgen-faster"
   exit 1;
fi
...
if [ -z "$si_dir" ]; then # we need to do the speaker-independent decoding pass.
  ...
  if [ $stage -le 0 ]; then
    ...
    steps/decode.sh --verbose $verbose --scoring-opts "$scoring_opts" \
...
if [ $stage -le 2 ]; then
  ...
  $cmd --num-threads $num_threads JOB=1:$nj $dir/log/decode.JOB.log \
    gmm-latgen-faster$thread_string --verbose=$verbose --max-active=$max_active --beam=$beam --lattice-beam=$lattice_beam \
