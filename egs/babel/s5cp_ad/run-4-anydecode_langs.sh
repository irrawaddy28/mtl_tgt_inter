#!/bin/bash 
# > ./run-4-anydecode.sh --skip-kws true --tri5-only true --dir dev10h.uem --lang BNG
...
if [ $# -ne 1 ]; then
  echo "Usage: $(basename $0) --type (dev10h|dev2h|eval|shadow) <language code>"
  exit 1
fi
...
function make_mfcc {	
  ...
  logdir=$2
  output=$3  
  steps/make_mfcc.sh --nj $my_nj --cmd "$train_cmd" $target $logdir $output
...
if [ ! -f data/$L/raw_${dataset_type}_data/.done ]; then
  echo ---------------------------------------------------------------------
  ...
  local/make_corpus_subset.sh $resource_string ./data/$L/raw_${dataset_type}_data
  touch data/$L/raw_${dataset_type}_data/.done
fi
my_data_dir=`readlink -f ./data/$L/raw_${dataset_type}_data`
...
if [ ! -f  $dataset_dir/.done ] ; then
  ...
  if [ ! -f ${dataset_dir}/.mfcc.done ]; then
    ...
    echo ---------------------------------------------------------------------
    make_mfcc ${dataset_dir} exp/$L/make_mfcc/${dataset_id} mfcc/$L
...
if ! $fast_path ; then
  local/run_kws_stt_task.sh --cer $cer --max-states $max_states \
    ...
    "${lmwt_plp_extra_opts[@]}" \
    ${dataset_dir} data/$L/lang ${decode}

  local/run_kws_stt_task.sh --cer $cer --max-states $max_states \
    --skip-scoring $skip_scoring --extra-kws $extra_kws --wip $wip \
    --cmd "$decode_cmd" --skip-kws $skip_kws --skip-stt $skip_stt  \
    "${lmwt_plp_extra_opts[@]}" \
    ${dataset_dir} data/$L/lang ${decode}.si
fi
