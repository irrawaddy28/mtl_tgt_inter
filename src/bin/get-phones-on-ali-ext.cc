...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    BaseFloat thresh = 0.0;
    ParseOptions po(usage);
    po.Register("thresh", &thresh, "posteriors below this value are discarded");
    po.Read(argc, argv);
    ...
    //kaldi::PosteriorWriter confidences_writer(confidences_wspecifier);
    ...
    for (; !posterior_reader.Done(); posterior_reader.Next()) {
      ...
      if (!alignments_reader.HasKey(key)) {
        ...
        snr(0) = confidence.Sum()/(num_frames - confidence.Sum());
        ...
        KALDI_LOG << key << ": # Signal Frames  = " << num_good_frames
				  << ", # Noise-only Frames    = " << num_frames - num_good_frames
				  << ", Total Frames = " << num_frames
				  << ", Signal = " << confidence.Sum()
				  << ", Noise  = " << num_frames - confidence.Sum()
